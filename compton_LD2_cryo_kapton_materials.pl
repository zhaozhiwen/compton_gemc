...
sub define_material
{
	...
	#unpol LD2 target (full)
	#2H
	%mat = init_mat();
	$mat{"name"}          = "2H_liquid";
	$mat{"description"}   = "2H_liquid";
	$mat{"density"}       = "0.1623";  #in g/cm3
	$mat{"ncomponents"}   = "1";
	$mat{"components"}    = "LD2 1";
	print_mat(\%configuration, \%mat);	
	
	#unpol LD2 target (empty) 
	#2H
	%mat = init_mat();
	$mat{"name"}          = "2H_gas";
	$mat{"description"}   = "2H_gas";
	$mat{"density"}       = "0.00234";  #in g/cm3
	$mat{"ncomponents"}   = "1";
	$mat{"components"}    = "LD2 1";
	print_mat(\%configuration, \%mat);	
